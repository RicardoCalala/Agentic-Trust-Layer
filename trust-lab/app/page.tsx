...
export default function Home() {
  ...
  const [riskAction, setRiskAction] = useState("Awaiting action");
  const [riskActionLog, setRiskActionLog] = useState<string[]>(["Simulation initialized — fictional aggregate inputs only"]);
  ...
  function runRiskAction(action: "intake" | "verify" | "review") {
    const actions = {
      intake: { stage: 0, label: "Aggregate intake refreshed", message: "New fictional aggregate intake was scoped to permitted use." },
      verify: { stage: 2, label: "Evidence verification complete", message: "Synthetic provenance and coverage checks were added to the review packet." },
      review: { stage: 4, label: "Human review prepared", message: "The fictional signal was routed to a qualified reviewer; no action against a person was taken." },
    } as const;
    const next = actions[action];
    setRiskAction(next.label);
    setRiskStage(next.stage);
    setRiskSeed(() => Math.floor(Math.random() * 900) + 100);
    setRiskProfileIndex(current => (current + 1) % riskProfiles.length);
    setRiskPulse(current => current + 1);
    setRiskActionLog(current => [next.message, ...current].slice(0, 4));
  }
